...
def create_topo(topo_info=(0, 4)):
    ...
    if topo_type == 0:
        ...
        new_topo = ftree_gen(topo_config)
    # add weights to links
    new_topo = add_weights(new_topo, 'equal')    
    return new_topo    
    
def ftree_gen(topo_config):
    ''' Generate Fat Tree topology '''
    num_pod = topo_config
    link_list = []
    top_switch = range(num_pod * num_pod, 5 * num_pod * num_pod / 4)
    for index in range(0, num_pod):
        base = index * num_pod / 2
        dst = range(base, base + num_pod / 2)
        print "dst", dst
        base2 = num_pod * num_pod / 2
        src = range(base + base2, base + base2 + num_pod/2)
        print "src", src
        for src_id in src:
            index_src = src.index(src_id)
            print index_src
            sub_top = top_switch[index_src * num_pod/2: (index_src + 1) *num_pod/2]
            print sub_top
            for bottom_dst in dst:
                link_list.append((src_id, bottom_dst))
            for top_dst in sub_top:
                link_list.append((src_id, top_dst))
    G = nx.Graph()
...
def create_option(parser):
    ...
    parser.add_option("--pod", dest="num_pods",
                      type="int",
                      default="4",
                      help="num of pods in the fat tree topology")
