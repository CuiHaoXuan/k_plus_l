...
for node in 22
do
	for load_type in morning afternoon evening
	do
		# first find feasible capacity
		if [ $load_type = morning ]
		then
			t=0930
		elif [ $load_type = afternoon ]
		then 
			t=1430
		elif [ $load_type = evening ]
		then
			t=2130
		fi
		capacity=4000
		...
		do
			...
			do	
				...
				echo "DAY, seed" $count $day
				json_file=$HomeD/topo_info/geant_demand/"geant_demand_2005050"$day'_'$t".json"
				echo $json_file
				if [ $dc_type = 'n' ] 
				then
				# for non-DC cases, the total demand pairs are C(n,2), where n is the number of nodes
				#d_pairs=`echo $node \\* \($node-1\) \\* 0.5 | bc -l`
					d_pairs=`( echo "import model_create"; echo "demand_dict = model_create.load_json('$json_file')"; echo "print len(demand_dict)" ) | python`
				...
				echo "seed # " $count
				obj_lb=lb
				echo "/bin/bash" $run_topo $obj_lb $node $num_paths $load_type $dc_type "no 4000" $cost_type $count
				echo `/bin/bash $run_topo $obj_lb $node $num_paths $load_type $dc_type no 4000 $cost_type $count`
