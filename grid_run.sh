...
# get number of nodes on the sides
side_nodes=`(echo "sqrt ( $num_nodes )" | bc -l) |awk '{printf("%.0f\n", $1)}'`
...
if [ $num_paths != 5 ]
then
	path_file=$path_dir/$topo_type'_'$side_nodes'_'$num_paths'path.txt'
else
	path_file=$path_dir/$topo_type'_'$side_nodes'_path.txt'
fi
...
csv_file=$csv_dir/$topo_type'_'$side_nodes'.csv'
...
echo `python $model_gen -w $result_folder/$sub_dir/$ampl_data \
		...
		--dc $dc_type --nn $num_nodes -c $cost_type --seed $seed`
