...
var x{d in demand_nos,p in route_nos,t in hours_nos} binary;
var r;
...
subject to path_constraint{d in demand_nos,t in hours_nos}:sum{p in route_nos}(x[d,p,t])=1; 

#formulation for minimizing link_utilization
subject to capacity_constraints{e in link_nos,t in hours_nos}:sum{d in demand_nos}sum{p in route_nos}(delta[e,d,p,t]*x[d,p,t]*h[d,t])
<=link_capacity[e]*r;
