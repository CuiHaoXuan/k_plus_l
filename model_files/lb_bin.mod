...
## -- Objective Equation --
minimize link_utilization: RR;

## -- Constraints --
subject to d_pair{d in demand}: sum{p in P[d]} x[d,p] = 1;

#subject to cl{l in links}: sum{d in demand, p in P[d]} delta[d,p,l] * h[d] * x[d,p] <= link_capacity[l]*RR;

subject to cl{l in links}: sum{d in demand, p in P[d]} delta[d,p,l] * h[d] * x[d,p] -yy[l] = 0;

subject to ycl{l in links}: yy[l] <= link_capacity[l]*RR;
